...
export default {
  ...
  mounted() {
    this.oWrap = this.$refs['article-list']
    this.oWrap.onscroll = throttle(() => {
      // 记录滚动的位置
      this.scrollValue = this.oWrap.scrollTop
    }, 100)
  },
  // 下一次返回到这里的时候，使用一下记录的位置
  activated() {
    this.oWrap.scrollTop = this.scrollValue
  }
}
</script>
